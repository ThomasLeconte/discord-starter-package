  ...
  listen() {
    this.bot.on("messageCreate", async (msg) => {
      ...
      if (this.bot.disabledCommands.has(command)) {
        this.bot.sendMessage(msg, ErrorEmbed(this.bot, `**${this.bot.name()} - Error**`, `The command "${command}" is disabled !`));
      } else {
        if (this.bot.commands.has(command)) {
          await this.bot.commands.get(command).execute(this.bot, msg, args).then((result: string | EmbedBuilder | MessageFormatter) => {
            ...
            this.bot.sendMessage(msg, ErrorEmbed(this.bot, `**${this.bot.name()} - Error**`, "An error has occured with this command. Please try again later ..."))
          });
        } else {
          this.bot.sendMessage(msg, ErrorEmbed(this.bot, `**${this.bot.name()} - Error**`, `The command "${command}" does not exist.`));
        }
      }
    });
  }
